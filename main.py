...
def send_to_openclaw(transcript: str, metadata: dict):
    ...
    message = (
        f"Voice note received from Derek. Process this and ALWAYS save to the Obsidian vault.\n\n"
        f"**Transcript:**\n> {transcript}\n\n"
        f"**Metadata:**\n"
        ...
        f"- Duration: {duration}\n\n"
        f"**CRITICAL RULES:**\n"
        f"1. Every voice note MUST result in a vault write. Derek records because he's away from his desk — if he recorded it, it matters.\n"
        f"2. Never just answer a question — always save the information to the appropriate vault note too.\n\n"
        f"**Step 1: Save raw transcript to Inbox/Voice/**\n"
        f"Create a note at `/home/node/obsidian-vault/Inbox/Voice/` with this exact frontmatter format:\n"
        f"```yaml\n"
        f"---\n"
        ...
        f"duration: \"{duration}\"\n"
        f"intent: <classify: new-knowledge | update-existing | action-item | person-context | question | random-thought | decision>\n"
        f"topics: [<relevant topics>]\n"
        f"routed_to: [<list of vault paths you created/updated>]\n"
        f"actions_taken: [<what you did>]\n"
        f"tags: [voice-note, <intent>, <topics>]\n"
        f"---\n"
        f"```\n\n"
        f"**Step 2: Route content to the right vault location**\n"
        f"- Search `/home/node/obsidian-vault` for related existing notes\n"
        f"- Create new notes or append to existing ones as appropriate\n"
        f"- Tasks/reminders → `/home/node/molty/TASKS.md` or relevant project checklist\n"
        f"- Family info → `Personal/Family/[person].md`\n"
        f"- Projects → `Projects/[area]/[topic].md`\n"
        f"- Research → `Research/[topic].md`\n"
        f"- Personal preferences → `Personal/About Derek.md`\n\n"
        f"**Step 3: Update the Inbox note**\n"
        f"After routing, update the Inbox/Voice/ note's frontmatter:\n"
        f"- Set `status: processed`\n"
        f"- Fill in `routed_to` with the paths you wrote to\n"
        f"- Fill in `actions_taken` with what you did\n\n"
        f"**Step 4: Confirm**\n"
        f"Send Derek a brief summary via Discord (channel: 1466806583714644063) confirming what was saved and where."
    )
